    ...
    def _get_token(self) -> Credentials:
        ...
        if not token or not token.valid:
            if token and token.expired and token.refresh_token:
                ...
                logger.info(f'Using OAuth redirect URI: http://localhost:{self.oauth_port}/')
...
async def main(creds_file_path: str,
    ...
    gmail_service = GmailService(creds_file_path, token_path, oauth_port=oauth_port)
    server = Server("gmail")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--oauth-port',
                        type=int,
                        default=8080,
                        help='Localhost OAuth redirect port. Must match Google OAuth redirect URI.')
    
    args = parser.parse_args()
    asyncio.run(main(args.creds_file_path, args.token_path, args.oauth_port))